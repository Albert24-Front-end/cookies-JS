...
// Фильтрация по признаку сахара
function switchSugarFree(products) {
    const sugarButton = document.querySelector(".sugar-free label");
    const pageContent = document.querySelector(".products .container");
    const sugarInfo = document.querySelectorAll(".products-item-info");
    sugarButton.addEventListener('click', ()=>{
        const filteredCookies = 
        sugarInfo.getAttribute("data-sugar-free") === "sugar-free" ? products.filter((product)=>{product.sugar.includes("sugar-free")}) : products;
        
        renderCookies(filteredCookies);
        pageContent.scrollIntoView({ behavior: "instant" });
    });
    document.addEventListener("DOMContentLoaded", ()=>{
        renderCookies(produc)
    })
}
