...
function fillInSelectedCookie (products) {
    ...
    orderButtons.forEach((button)=>{
        ...
        button.addEventListener('click', (event)=>{
            // inputCookie.value = cookieTitle.innerText;
            // Находим карточку, содержащую нажатую кнопку
            const productItem = event.target.closest(".products-item");

            // Получаем заголовок товара из этой карточки
            const cookieTitle = productItem.querySelector(".products-item h4").textContent;
            console.log(cookieTitle)
        });
    
    });
};
fillInSelectedCookie(products);
